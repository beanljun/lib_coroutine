...
static int real_daemon(int argc, char** argv, std::function<int(int argc, char** argv)> main_cb) {
    ...
    daemon(int nochdir, int noclose)
        ...
        noclose：
            如果为 0（在这里是 0），将标准输入、标准输出和标准错误重定向到
    /dev/null。 如果非 0，保持这些文件描述符不变。 daemon(1, 0) 的效果是：
        保持当前工作目录不变。将标准输入、标准输出和标准错误重定向到 /dev/null。

    调用 daemon(1, 0) 后，进程会经历以下变化：
        1、创建一个新的会话（session），使进程脱离控制终端。
        2、将进程的工作目录设置为根目录（但在这里因为第一个参数是
    1，所以不改变）。 3、重设文件创建掩码（umask）为 0。
        4、关闭所有打开的文件描述符（但在这里标准输入、输出和错误会重定向到
    /dev/null）。 目的： 1、使进程在后台运行，不受终端的影响。
        2、确保进程不会因为终端关闭而被终止。
