...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    //         SYLAR_LOG_INFO(g_logger) << "content_len=" <<
    //         client_parser.content_len; if(client_parser.content_len <= len) {
    ...
    //             while(left > 0) {
    //                 int rt = read(data, left > (int)buff_size ?
    //                 (int)buff_size : left); if(rt <= 0) {
    //                     close();
    //                     return nullptr;
    //                 }
    //                 body.append(data, rt);
