...
void LogFormatter::init() {
    ...
    std::string
         dateformat;  /// 日期格式字符串，默认把位于%d后面的大括号对里的全部字符都当作格式字符，不校验格式是否合法
    ...
    while (i < m_pattern.size()) {
        ...
        if (c == "%") {
            if (parsing_string) {
                ...
                patterns.emplace_back(std::make_pair(1, c));
                parsing_string = true;
                i++;
                continue;
            }
...
#undef XX
    ...
    for (auto& v : patterns) {
        if (v.first == 0)
            ...
            m_items.emplace_back(FormatItem::ptr(new DateTimeFormatItem(dateformat)));
        else {
            ...
            if (it == s_format_items.end()) {
                ...
                m_items.emplace_back(it->second(v.second));
        }
    }
        ...
        if (node["appenders"].IsDefined()) {
            ...
            for (size_t i = 0; i < node["appenders"].size(); i++) {
                ...
                if (type == "FileLogAppender") {
                    ...
                    // 若没有设置文件路径，输出错误日志
                    if (!a["file"].IsDefined()) {
                        std::cout << "log appender config error : file "
                                     "appender is null, "
                                  << a << std::endl;
                        continue;
                    }
                    ...
                    std::cout << "log appender config error : appender type is "
                                 "invalid, "
                              << a << std::endl;
                    continue;
                }
    ...
    LogIniter() {
        ...
        g_log_defines->addListener([](const std::set<LogDefine>& old_value, const std::set<LogDefine>& new_value) {
            ...
            for (auto& i : new_value) {
                ...
                if (it == old_value.end())
                    logger = SYLAR_LOG_NAME(i.name);  // 新增logger
                else {
                    if (!(i == *it))
                        logger = SYLAR_LOG_NAME(i.name);  // 修改的logger
                    else
                        continue;
                }
                ...
                for (auto& a : i.appenders) {
                    ...
                    if (a.type == 1)
                        ap.reset(new FileLogAppender(a.file));  // FileLogAppender
