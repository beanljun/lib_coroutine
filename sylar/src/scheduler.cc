...
//
//  step 3
//  如果`use_caller`为`false`，则将`m_rootThread`设置为-1，表示没有使用调用者线程。
//
...
Scheduler::Scheduler(size_t threads, bool use_caller, const std::string &name) {
    ...
    if (use_caller) {
         ...
         *
         * std::bind(&Scheduler::run,
         * this)生成了一个新的可调用对象，该对象绑定了成员函数`Scheduler::run`和对象`this`。
...
// step 1
// 检查`m_stopping`成员变量，如果它为`true`，则说明调度器正在停止，此时直接返回。
// step 2
// 确保`m_threads`向量是空的，也就是说，确保调度器还没有启动，调整`m_threads`向量的大小为`m_threadCount`，也就是要创建的线程数量。
// step 3
// 创建线程，线程对象被存储在`m_threads`向量中，线程的ID被添加到`m_threadIds`向量中。
...
void Scheduler::run() {
    ...
    setThis();
    // 如果当前线程不是调度线程，则将当前线程的调度协程设置为当前线程的主协程
    if (sylar::GetThreadId() != m_rootThread)
        t_scheduler_fiber = sylar::Fiber::GetThis().get();

    // 创建一个idle协程，使用bind()将Scheduler::idle()函数绑定到idle协程上
    ...
    while (true) {
        ...
        bool tickle_me = false;  // 是否唤醒其他线程进行任务调度
        {
            ...
            while (it != m_tasks.end()) {
                ...
                // if(it -> fiber) SYLAR_ASSERT(it -> fiber -> getState() ==
                // Fiber::READY); [BUG FIX]: hook
        ...
        if (task.fiber) {
            ...
            if (cb_fiber)
                cb_fiber->reset(task.cb);  // 重置 cb_fiber 并设置其回调函数为 task.cb
            else
                cb_fiber.reset(new Fiber(task.cb));  // 创建一个新的 Fiber 对象，其回调函数为 task.cb
            task.reset();
