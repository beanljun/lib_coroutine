...
static int32_t DecodeZigzag32(const uint32_t& v) {
    ...
    //              对于负数 -n，编码为 2n-1，解码时右移一位得到 n-1，再通过或
    //              -1 操作得到 -(n-1)-1，即 -n
    // v >> 1:
    // 将v右移1位，相当于除以2，对于偶数编码（原始非负数），会得到原始值，对于奇数编码（原始负数），这会得到原始负数的绝对值减
    // 1
    // -(v & 1): 如果v是偶数，则结果为0，如果v是奇数，则结果为-1
    // (v >> 1) ^ -(v & 1): 对于原始非负数，-(v & 1) 为 0，异或操作不改变 v >> 1
    // 的值
    //                      对于原始负数，-(v & 1) 为 -1（所有位为
    //                      1），异或操作会将 v >> 1 的所有位取反并减 1
    return (v >> 1) ^ -(v & 1);  // 很精妙！！！
}
...
void ByteArray::writeUint32(uint32_t value) {
    ...
    while (value >= 0x80) {                // 如果value >=
                                           // 0x80，则需要将value分成多个字节写入到ByteArray中
