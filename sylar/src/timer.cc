...
bool Timer::Comparator::operator()(const Timer::ptr& lhs, const Timer::ptr& rhs) const {
    if (lhs == rhs)
        return false;
    if (!lhs || lhs->m_next < rhs->m_next)
        return true;
    if (!rhs || rhs->m_next < lhs->m_next)
        return false;
...
bool Timer::reset(uint64_t ms, bool from_now) {
    if (ms == m_ms && !from_now)
        return true;  // 如果定时器执行周期和当前周期相同，并且不是从当前时间开始计算，已经是最新的定时器，直接返回true
    TimerManager::RWMutexType::WriteLock lock(m_manager->m_mutex);
    if (!m_cb)
        return false;  // 如果回调函数为空，直接返回
    ...
    uint64_t start_t = 0;
    if (from_now)
        start_t = sylar::GetElapsedMS();  // step3:
                                          // 如果是从当前时间开始计算，直接使用当前时间
    else
        start_t = m_next - m_ms;  // 否则，重新计算执行时间
    m_ms = ms;
...
Timer::ptr TimerManager::addConditionTimer(uint64_t              ms,
                                           ...
                                           bool                  recurring) {
    // 在定时器触发时会调用 OnTimer
    // 函数，并在OnTimer函数中判断条件对象是否存在，如果存在则调用回调函数cb
...
void TimerManager::listExpiredCb(std::vector<std::function<void()>>& cbs) {
    ...
    // 否则返回第一个 > now_timer的迭代器,
    // 在此迭代器之前的Timer都是已经超时的Timer
    auto                    end = rollover ? m_timers.end() : m_timers.upper_bound(now_timer);
    std::vector<Timer::ptr> expired(m_timers.begin(),
                                    end);   // 将已经超时的Timer添加到expired中
    m_timers.erase(m_timers.begin(), end);  // 将已经放入expired的定时器删掉
...
void TimerManager::addTimer(Timer::ptr val, RWMutexType::WriteLock& lock) {
    auto it = m_timers.insert(val).first;  // 将定时器添加到定时器集合中,
                                           // .first表示返回一个pair，pair的first表示定时器的迭代器
    bool at_front = (it == m_timers.begin()) && !m_tickled;  // 判断是否是最早执行的定时器
    if (at_front)
        m_tickled = true;  // 如果是最早执行的定时器，设置m_tickled为true
    lock.unlock();         // 解锁
    if (at_front)
