...
bool Address::Lookup(std::vector<Address::ptr> &result, const std::string &host, int family, int type, int protocol) {
    ...
    // 遍历结果，将结果添加到result中
    while (rp != nullptr) {
        result.emplace_back(Create(rp->ai_addr, static_cast<socklen_t>(rp->ai_addrlen)));
...
bool Address::operator<(const Address &rhs) const {
    ...
    // memcmp: 比较两个内存区域的前minlen个字节, 返回值<0, 0,
    // >0分别表示小于，等于，大于 0x00000001 < 0x00000002 0x00000001 ==
...
IPAddress::ptr IPv4Address::BroadcastAddress(uint32_t prefix_len) {
    ...
    baddr.sin_addr.s_addr |=
        byteswapOnLittleEndian(CreateMask<uint32_t>(prefix_len));  //创建掩码，如24位，二进制形式：11111111
...
// 2001:0db8:85a3:0000:0000:8a2e:0370:7334端口8080 ->
