...
unsigned int sleep(unsigned int seconds) {
     ...
     *sylar::Fiber::ptr 和 int 类型的成员函数的指针类型。
     *	具体来说，它的含义如下：
     *	void 表示该成员函数的返回值类型，这里是 void 类型。
     *	(sylar::Scheduler::*) 表示这是一个 sylar::Scheduler
     *类的成员函数指针类型。 (sylar::Fiber::ptr, int thread)
     *表示该成员函数的参数列表 ，其中第一个参数为 sylar::Fiber::ptr
     *类型，第二个参数为 int 类型。
     *
     *	使用 std::bind 绑定了 sylar::IOManager::schedule 函数，
     * 	并将 iom 实例作为第一个参数传递给了 std::bind
     *函数，将sylar::IOManager::schedule函数绑定到iom对象上。
     * 	在这里，第二个参数使用了函数指针类型
     *(void(sylar::Scheduler::*)(sylar::Fiber::ptr, int thread))
     * 	，表示要绑定的函数类型是 sylar::Scheduler 类中一个参数为
     *sylar::Fiber::ptr 和 int 类型的成员函数 ，这样 std::bind
     *就可以根据这个函数类型来实例化出一个特定的函数对象，并将 fiber 和 -1
     *作为参数传递给它。
     */
    iom->addTimer(
        seconds * 1000,
        std::bind(
            (void (sylar::Scheduler::*)(sylar::Fiber::ptr, int thread)) & sylar::IOManager::schedule, iom, fiber, -1));
    sylar::Fiber::GetThis()->yield();
    return 0;
}
