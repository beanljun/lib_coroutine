    ...
    */
    ...
    void LogFormatter::init() {
        ...
        while(i < m_pattern.size()) {
            ...
            if(c == "%") {
                if(parsing_string) {
                    if(!tmp.empty())  patterns.emplace_back(std::make_pair(0, tmp));
                    tmp.clear();
...
#undef XX
        ...
        for(auto& v : patterns) {
            if( v.first == 0) m_items.emplace_back(FormatItem::ptr(new StringFormatItem(v.second)));
            else if(v.second == "d") m_items.emplace_back(FormatItem::ptr(new DateTimeFormatItem(dateformat)));
            else {
                ...
                if(it == s_format_items.end()) {
                    ...
                    error = true;
                    break;
                } else m_items.emplace_back(it -> second(v.second));
            }
        }
